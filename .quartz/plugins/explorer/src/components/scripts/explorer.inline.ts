...
function renderTree(node, container, currentSlug, folderBehavior, savedState, pathPrefix = "") {
  ...
  if (node.isFolder) {
    ...
    if (link) {
      ...
      const titleSpan = link.querySelector(".file-title-text");
      if (titleSpan) {
        titleSpan.textContent = node.displayName || node.slugSegment;
      } else {
...
async function handleNavOrRender(e) {
  ...
  try {
    ...
    for (const explorer of allExplorers) {
      ...
      const folderIcons = explorer.getElementsByClassName("folder-collapse-icon");
...
// --- 新建按钮交互 ---
function setupNewNoteButton(explorer) {
  const newButton = explorer.querySelector(".new-note-button")
  if (!newButton) return

  const panelTemplate = document.getElementById("template-new-note-panel")
  ...
  newButton.addEventListener("click", (e) => {
    ...
    const panel = clone.querySelector(".new-note-panel")
    if (!panel) return
...
// Initialize new-note buttons on nav (avoid duplicate binding)
document.addEventListener("nav", () => {
  for (const explorer of document.querySelectorAll("div.explorer")) {
    if (!explorer.hasAttribute("data-new-note-bound")) {
      explorer.setAttribute("data-new-note-bound", "true")
      setupNewNoteButton(explorer)
    }
  }
})
